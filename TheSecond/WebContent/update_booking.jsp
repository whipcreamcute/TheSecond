...
<title>Update Booking | The Second</title>
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-end">
								<li class="breadcrumb-item"><a href="../bookings.jsp"><i
										class="bi bi-book"></i> Bookings</a></li>
							</ol>
						</div>
					</div>
					<!--end::Row-->
				...
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-6">
							...
							if (bookingId > 0) {
								book = service.getBooking(bookingId);
							}

							ArrayList<Room> roomList = roomService.getActiveRooms();
							// เก็บลงใน Scope เพื่อให้ JSTL เรียกใช้ได้ 
										...
										<div class="row mb-3">
											...
											<div class="col-sm-10">
												<input type="hidden" name="userId" value="${userId}" /> <input
													type="hidden" name="bookingId" value="${book.getId()}" /> <select
													name="roomId" class="form-select" id="roomId">
													<c:if test="${not empty roomList}">
														<c:forEach var="room" items="${roomList}" varStatus="loop">
															<option value="${room.getId()}"
																<c:if test="${room.getId() == book.getRoomId()}">
								  selected
							   </c:if>>${room.getRoomName()}</option>
														</c:forEach>
													</c:if>
												</select>
											</div>
										...
										<div class="row mb-3">
											<label for="bookingDate" class="col-sm-2 col-form-label">Date
											</label>
											<div class="col-sm-10">
												<input type="date" min="2025-01-01" max="2026-12-31" class="form-control" style="width: 190px"
										...
										<div class="row mb-3">
											<label for="bookingTime" class="col-sm-2 col-form-label">Time</label>
											<div class="col-sm-10">
										...
										<div class="row mb-3">
											<label for="bookingTime" class="col-sm-2 col-form-label">Note</label>
											<div class="col-sm-10">
												<textarea cols="80" rpws="2" class="form-control"
													name="note" id="note">${book.getRemark()}</textarea>
											</div>
										</div>
									...
									<div class="card-footer">
										...
										<a href="../bookings.jsp" class="btn float-end">Cancel</a>
									</div>
									<!--end::Footer-->
								</form>
								<!--end::Form-->
